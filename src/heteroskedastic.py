...
This module file is modified from the orginial
https://github.com/dmilios/dirichletGPC/blob/master/src/heteroskedastic.py
to be executed using python 3.9 or newer and GPflow 2x
'''
...
# Defines a class SGPRh, which inherits from the GPModel class, representing a Sparse Gaussian Process Regression model
class SGPRh(GPModel, InternalDataTrainingLossMixin):
    def __init__(self, data, kernel, sn2, Z, mean_function=None, **kwargs):
        """
        X is a data matrix, size N x D: N num of data points, D num of features
        Y is a data matrix, size N x R: R num of labels
        Z is a matrix of pseudo inputs, size M x D: M num pf pseudo inputs
    ...
    def elbo(self):
        ...
        kuu = Kuu(Z, self.kernel, jitter=1e-4)
